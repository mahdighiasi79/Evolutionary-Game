...
class Player():
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        for box_list in box_lists:
            if append == 3:
                break
            if box_list.x > agent_x:
                network_input.append((box_list.x - agent_x) / width)
                network_input.append((box_list.gap_mid - agent_y) / height)
                append += 1
        for i in range(append, 3):
            network_input.append(1)
            network_input.append(0.5)
        network_input.append(velocity / 9.8)

        decision = self.nn.forward(network_input)
